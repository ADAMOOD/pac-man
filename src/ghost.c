...
void renderGhost(SDL_Renderer *renderer, Ghost *ghost, Map m)
{
    ...
    if (ghost->texture != NULL)
    {
        ...
        {
        case UP:
            srcRect.x = ghost->currentFrame % 2 == 0 ? ghost->frameWidth : 0;  // Alternating frames for UP direction (1,2)
            break;
        case DOWN:
            srcRect.x = ghost->currentFrame % 2 == 0 ? ghost->frameWidth * 3 : ghost->frameWidth * 2;  // Alternating frames for DOWN direction (7,8)
            break;
        case LEFT:
            srcRect.x = ghost->currentFrame % 2 == 0 ? ghost->frameWidth * 2 : ghost->frameWidth;  // Alternating frames for LEFT direction (3,4)
            break;
        case RIGHT:
            srcRect.x = ghost->currentFrame % 2 == 0 ? 0 : ghost->frameWidth;  // Alternating frames for RIGHT direction (1,2)
            break;
        default:
            break;
        }
